...
## Recent Changes
- **Feb 26, 2026**: Police divisions (PST, RPT, CIB, Dogs Squad) - Division sub-tab in police roster; leadership can assign/remove divisions via dropdown; Division shown in PlayerCardDialog; `division` column on roster_members table. Leadership notes on player profiles - `roster_notes` table; notes visible only to department leadership + director/executive/manager (NOT visible to player); add/delete notes from PlayerCardDialog. Department application form open/close toggle - `isOpen` column on application_forms; leadership can toggle forms open/closed; closed forms show red "Closed" badge and lock icon; submissions blocked when form is closed. Fixed leadership access caching bug (Departments.tsx checkAccess now returns full AccessData matching DepartmentPortal query shape, preventing staleTime:Infinity from caching wrong type).
