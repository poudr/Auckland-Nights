...
export class DatabaseStorage implements IStorage {
  ...
  async getRosterNotes(rosterId: string, departmentCode: string): Promise<RosterNote[]> {
    return await db.select().from(rosterNotes)
      .where(and(eq(rosterNotes.rosterId, rosterId), eq(rosterNotes.departmentCode, departmentCode)))
